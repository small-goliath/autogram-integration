...
export default function SnsRaiseUserManager() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const res = await fetchWithAdminAuth('/api/admin/users');
  ...
  const addUser = async (e: FormEvent) => {
    ...
    try {
      const res = await fetchWithAdminAuth(`/api/admin/users/${newUser}`, {
        method: 'POST',
      });
      if (!res.ok) {
  ...
  const deleteUser = async (username: string) => {
    ...
    try {
      const res = await fetchWithAdminAuth(`/api/admin/users/${username}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
        const errData = await res.json();
